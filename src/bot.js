...
bot.command('start', async (ctx) => {
  try {
    ...
    const welcomeMessage = `🤖 Welcome to Agent 47 - Solana Trading Bot!\n\n` +
...
bot.command('wallet', async (ctx) => {
  try {
    ...
    // Get balance
    const balance = await getWalletBalance(user.wallet_address, SOLANA_CONNECTION);
...
bot.command('copytrade', async (ctx) => {
  try {
    ...
    if (NETWORK !== 'devnet') {
      await ctx.reply('❌ Copy trading is only available on devnet for safety.');
      return;
    }
...
bot.command('mycopies', async (ctx) => {
  try {
    ...
    const message = `🐋 Your Copy Trades\n\n` +
...
bot.on('text', async (ctx) => {
  try {
    ...
    const swapMatch = text.match(/^(\d+\.?\d*)\s+(SOL|USDC)\s+to\s+(SOL|USDC)$/i);
    if (swapMatch) {
      if (!user || !user.wallet_address) {
        await ctx.reply('❌ You don\'t have a wallet yet. Use /createwallet to create one.');
        return;
      ...
      await ctx.reply(
        ...
        Markup.inlineKeyboard([
          [
            Markup.button.callback('✅ Confirm', `swap_confirm_${amount}_${fromToken}_${toToken}`),
    ...
    const sendMatch = text.match(/^(\d+\.?\d*)\s+(SOL|USDC)\s+to\s+([1-9A-HJ-NP-Za-km-z]{32,44})$/i);
    if (sendMatch) {
      if (!user || !user.wallet_address) {
        await ctx.reply('❌ You don\'t have a wallet yet. Use /createwallet to create one.');
        return;
      ...
      await ctx.reply(
        ...
        Markup.inlineKeyboard([
          [
            Markup.button.callback('✅ Confirm', `send_confirm_${amount}_${token}_${recipientAddress}`),
    ...
    if (offrampMatch) {
      ...
      // Estimate Naira
      const estimatedNaira = await estimateNaira(token === 'USDC' ? amount : amount * 100);
      ...
      await ctx.reply(
        ...
        Markup.inlineKeyboard([
          [
            Markup.button.callback('✅ Confirm', `offramp_confirm_${amount}_${token}`),
    ...
    if (bankMatch) {
      ...
      await saveBankAccount(telegramId, bankName, accountNumber, accountName, db);
    ...
    const copyTradeMatch = text.match(/^([1-9A-HJ-NP-Za-km-z]{32,44})\s+(\d+\.?\d*)$/);
    if (copyTradeMatch) {
      if (!user || !user.wallet_address) {
        await ctx.reply('❌ You don\'t have a wallet yet. Use /createwallet to create one.');
        return;
    ...
    const spawnMatch = text.match(/^(trader|analyst|sniper|liquidity)\s+(\w+)\s+(\{.+\})$/);
    if (spawnMatch) {
      if (!user || !user.wallet_address) {
        await ctx.reply('❌ You don\'t have a wallet yet. Use /createwallet to create one.');
        return;
...
bot.action(/^swap_confirm_(.+)_(.+)_(.+)$/, async (ctx) => {
  try {
    ...
    console.log(`Swap confirmation from ${telegramId}: ${amount} ${fromToken} to ${toToken}`);
...
bot.action(/^send_confirm_(.+)_(.+)_(.+)$/, async (ctx) => {
  try {
    ...
    console.log(`Send confirmation from ${telegramId}: ${amount} ${token} to ${recipientAddress}`);
