...
function withUser(req, res, next) {
  ...
  const user = db
    .prepare("SELECT * FROM users WHERE dashboard_token = ?")
    .get(token);
...
app.get("/api/me", withUser, async (req, res) => {
  const u = req.user;
  const balance = u.wallet_address
    ? await fetchBalance(u.wallet_address)
    : null;
  res.json({
...
app.get("/api/me/overview", withUser, (req, res) => {
  ...
  const totalTxns = q(
    "SELECT COUNT(*) as c FROM transactions WHERE telegram_id=?",
  );
  const successTxns = q(
    "SELECT COUNT(*) as c FROM transactions WHERE telegram_id=? AND status='success'",
  );
  const swapCount = q(
    "SELECT COUNT(*) as c FROM transactions WHERE telegram_id=? AND type='swap'",
  );
  const transferCount = q(
    "SELECT COUNT(*) as c FROM transactions WHERE telegram_id=? AND type='transfer'",
  );
  const offrampCount = q(
    "SELECT COUNT(*) as c FROM transactions WHERE telegram_id=? AND type='offramp'",
  );
  const copyCount = q(
    "SELECT COUNT(*) as c FROM transactions WHERE telegram_id=? AND type='copy_trade'",
  );
  const activeCopies = q(
    "SELECT COUNT(*) as c FROM copy_trades WHERE telegram_id=? AND is_active=1",
  );
  const totalCopies = q(
    "SELECT COUNT(*) as c FROM copy_trades WHERE telegram_id=?",
  );
  const runningAgents = q(
    "SELECT COUNT(*) as c FROM agent_bots WHERE telegram_id=? AND is_running=1",
  );
  const totalAgents = q(
    "SELECT COUNT(*) as c FROM agent_bots WHERE telegram_id=?",
  );
  const swapVol = qv(
    "SELECT COALESCE(SUM(amount),0) as v FROM transactions WHERE telegram_id=? AND type IN ('swap','copy_trade') AND status='success'",
  );
  const transferVol = qv(
    "SELECT COALESCE(SUM(amount),0) as v FROM transactions WHERE telegram_id=? AND type='transfer' AND status='success'",
  );
  const offrampVol = qv(
    "SELECT COALESCE(SUM(amount),0) as v FROM transactions WHERE telegram_id=? AND type='offramp' AND status='success'",
  );
  const activity = db
    .prepare(
      `
...
app.get("/api/me/transactions", withUser, (req, res) => {
  ...
  const total = db
    .prepare(`SELECT COUNT(*) as c FROM transactions ${where}`)
    .get(...args).c;
...
app.get("/api/me/copy-trades", withUser, (req, res) => {
  const rows = db
    .prepare(
      "SELECT * FROM copy_trades WHERE telegram_id=? ORDER BY created_at DESC",
    )
...
app.get("/api/me/agents", withUser, (req, res) => {
  const rows = db
    .prepare(
      "SELECT * FROM agent_bots WHERE telegram_id=? ORDER BY created_at DESC",
    )
    .all(req.user.telegram_id);
  res.json(
    rows.map((r) => {
...
app.get("/api/me/bank", withUser, (req, res) => {
  const row = db
    .prepare("SELECT * FROM bank_accounts WHERE telegram_id=?")
