...
function configPills(obj) {
  if (!obj || typeof obj !== "object")
    return '<span class="cell-dim">' + (obj || "--") + "</span>";
  return Object.entries(obj)
    .map(function (e) {
      return (
        '<span class="config-pill"><span class="config-key">' +
        e[0] +
        ':</span> <span class="config-val">' +
        e[1] +
        "</span></span>"
      );
    })
...
function emptyRow(cols, msg) {
  ...
  return (
    '<tr><td colspan="' +
    cols +
    '" style="text-align:center;padding:40px 0;color:var(--text-3);font-size:13px;">' +
    msg +
    "</td></tr>"
  );
}
...
function showApp() {
  ...
  $("screenInvalid").style.display = "none";
  $("app").style.display = "flex";
}
function showInvalid() {
  $("screenLoading").style.display = "none";
  $("screenInvalid").style.display = "flex";
...
function renderActivityChart(activity) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const found = activity.find(function (a) {
      return a.day === key;
    });
    days.push({
      txns: found ? found.txns : 0,
      label: d.toLocaleDateString("en-GB", { day: "2-digit", month: "short" }),
    });
  }
  const maxTxns = Math.max.apply(
    null,
    days
      .map(function (d) {
        return d.txns;
  ...
  if (
    ...
    chart.innerHTML =
      '<div style="color:var(--text-3);font-size:12px;font-family:var(--mono);padding:20px 0">No activity in last 7 days</div>';
    return;
  }
...
function renderRecentTxns(rows) {
  if (!rows.length) {
    setHTML("recentTxBody", emptyRow(5, "No transactions yet"));
    return;
  }
  setHTML(
    "recentTxBody",
    rows
      .map(function (r) {
        return (
          ...
          "<td>" +
          (r.token_in
            ? '<span class="badge badge-gray">' + r.token_in + "</span>"
            : "") +
          (r.token_out
            ? '<span class="badge badge-gray">' + r.token_out + "</span>"
            : "") +
          "</td>" +
          "<td>" +
...
function updateTxnPagination() {
  ...
  setText(
    "txnPagInfo",
    txnTotal ? start + "-" + end + " of " + txnTotal : "No records",
  );
  $("txnPrev").disabled = txnOffset === 0;
  $("txnNext").disabled = txnOffset + TXN_LIMIT >= txnTotal;
}

$("txnPrev").addEventListener("click", function () {
  txnOffset = Math.max(0, txnOffset - TXN_LIMIT);
  loadTransactions(false);
});
$("txnNext").addEventListener("click", function () {
  txnOffset += TXN_LIMIT;
  loadTransactions(false);
});
["txnTypeFilter", "txnStatusFilter"].forEach(function (id) {
...
async function loadCopyTrade() {
  ...
  const txns = results[1];
  if (!rows) {
    toast("Failed to load copy trades", "error");
    return;
  }
  allCopyTrades = rows;
  setText(
...
function renderCopyTrades(rows) {
  ...
  const filtered =
    sf !== ""
      ? rows.filter(function (r) {
          return String(r.is_active) === sf;
        })
      : rows;
  setText("ctCount", filtered.length);
  if (!filtered.length) {
    setHTML("ctBody", emptyRow(5, "No copy trades configured yet"));
    return;
  }
  setHTML(
    "ctBody",
    filtered
      .map(function (r) {
        return (
          ...
          "</td>" +
          "<td>" +
          addrEl(r.target_wallet) +
          "</td>" +
          '<td class="cell-mono">' +
          fmtSol(r.max_sol_per_trade) +
          " SOL</td>" +
          "<td>" +
          activeTag(r.is_active) +
          "</td>" +
          '<td class="cell-dim">' +
          fmtDate(r.created_at) +
          "</td>" +
...
$("ctStatusFilter").addEventListener("change", function () {
  renderCopyTrades(allCopyTrades);
});
...
function renderAgents(rows) {
  ...
  if (tf)
    f = f.filter(function (r) {
      return r.agent_type === tf;
    });
  if (sf !== "")
    f = f.filter(function (r) {
      return String(r.is_running) === sf;
    });
  setText("agCount", f.length);
  if (!f.length) {
    setHTML("agBody", emptyRow(6, "No agent bots created yet"));
    return;
  }
  setHTML(
    "agBody",
    f
      .map(function (r) {
        return (
          ...
          "</td>" +
          "<td>" +
          badgeAgentType(r.agent_type) +
          "</td>" +
          '<td style="max-width:300px">' +
          configPills(r.config) +
          "</td>" +
          "<td>" +
          runningTag(r.is_running) +
          "</td>" +
          '<td class="cell-dim">' +
          fmtDate(r.created_at) +
          "</td>" +
...
["agTypeFilter", "agStatusFilter"].forEach(function (id) {
  $(id).addEventListener("change", function () {
    renderAgents(allAgents);
  });
});
...
async function loadOfframp() {
  ...
  if (!rows.length) {
    setHTML("orBody", emptyRow(6, "No off-ramp transactions yet"));
    return;
  }
  setHTML(
    "orBody",
